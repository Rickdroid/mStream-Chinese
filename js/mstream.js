  ...
  return '\
    <div class="clear relative">\
      ...
      <div class="song-button-box">\
        ...
        <span data-directory="' + dataDirectory + '" title="下载播放列表" class="downloadFileplaylist">\
...
$(document).ready(function () {
  ...
  function loadFileExplorer() {
    ...
    resetPanel('文件管理', 'scrollBoxHeight1');
    programState = [{
      state: 'fileExplorer'
    }];
    $('#directory_bar').show();
  ...
  function getAllPlaylists(previousState) {
    ...
    resetPanel('播放列表', 'scrollBoxHeight1');
    $('#filelist').html('<div class="loading-screen"><svg class="spinner" width="65px" height="65px" viewBox="0 0 66 66" xmlns="http://www.w3.org/2000/svg"><circle class="spinner-path" fill="none" stroke-width="6" stroke-linecap="round" cx="33" cy="33" r="30"></circle></svg></div>');
    currentBrowsingList = [];
  ...
  $('.mobile-panel').on('click', function () {
    ...
    resetPanel('手机软件', 'scrollBoxHeight2');
    $('#directory_bar').hide();
    ...
    $('#filelist').html("\
      ...
      <div class='app-text'>\
        官方mStream App现在可用于安卓手机上. 用他来同步你的mStream服务器到手机上吧。\
        <br><br>\
        IOS版本将会在不久的将来上线.\
        <br><br>\
        <a target='_blank' href='/public/qr-tool.html'>查看二维码，用于帮助你将服务器添加到你的手机app里</a>\
      </div>\
    ");
  });
  ...
  $('.db-panel').on('click', function () {
    ...
    resetPanel('数据库', 'scrollBoxHeight2');
    $('#filelist').html('<div class="loading-screen"><svg class="spinner" width="65px" height="65px" viewBox="0 0 66 66" xmlns="http://www.w3.org/2000/svg"><circle class="spinner-path" fill="none" stroke-width="6" stroke-linecap="round" cx="33" cy="33" r="30"></circle></svg></div>');
    $('#directory_bar').hide();
    currentBrowsingList = [];
    ...
    MSTREAMAPI.dbStatus(function (response, error) {
      ...
      $('#filelist').html('<p>你的数据库有 ' + response.totalFileCount + ' 个文件</p><input type="button" value="建立数据库" id="build_database">');
    });
  });
  ...
  function getRecentlyAdded() {
    ...
    resetPanel('最近添加', 'scrollBoxHeight1');
    $('#filelist').html('<div class="loading-screen"><svg class="spinner" width="65px" height="65px" viewBox="0 0 66 66" xmlns="http://www.w3.org/2000/svg"><circle class="spinner-path" fill="none" stroke-width="6" stroke-linecap="round" cx="33" cy="33" r="30"></circle></svg></div>');
    $('.directoryName').html('获取最近 &nbsp;&nbsp;<input id="recently-added-limit" class="recently-added-input" type="number" min="1" step="1" value="100">&nbsp;&nbsp; 首歌');
  ...
  function getAllAlbums(previousState) {
    ...
    resetPanel('专辑', 'scrollBoxHeight1');
    $('#filelist').html('<div class="loading-screen"><svg class="spinner" width="65px" height="65px" viewBox="0 0 66 66" xmlns="http://www.w3.org/2000/svg"><circle class="spinner-path" fill="none" stroke-width="6" stroke-linecap="round" cx="33" cy="33" r="30"></circle></svg></div>');
    currentBrowsingList = [];
  ...
  function getAllArtists(previousState) {
    ...
    resetPanel('艺术家', 'scrollBoxHeight1');
    $('#filelist').html('<div class="loading-screen"><svg class="spinner" width="65px" height="65px" viewBox="0 0 66 66" xmlns="http://www.w3.org/2000/svg"><circle class="spinner-path" fill="none" stroke-width="6" stroke-linecap="round" cx="33" cy="33" r="30"></circle></svg></div>');
    currentBrowsingList = [];
  ...
  function getRatedSongs() {
    ...
    resetPanel('星标', 'scrollBoxHeight1');
    $('#filelist').html('<div class="loading-screen"><svg class="spinner" width="65px" height="65px" viewBox="0 0 66 66" xmlns="http://www.w3.org/2000/svg"><circle class="spinner-path" fill="none" stroke-width="6" stroke-linecap="round" cx="33" cy="33" r="30"></circle></svg></div>');
    $('#search_folders').val('');
    currentBrowsingList = [];
  ...
  function setupSearchPanel(searchTerm) {
    ...
    resetPanel('搜索数据库', 'scrollBoxHeight1');
    currentBrowsingList = [];
    $('#directory_bar').show();
    ...
    var newHtml = 
      '<div>\
        <form id="db-search" onsubmit="return false;">\
          <input ' + valString + ' id="search-term" required type="text" placeholder="搜索数据库">\
        ...
        <input ' + (searchToggles.artists === true ? 'checked' : '') + ' id="search-in-artists" type="checkbox"><label for="search-in-artists">艺术家</label>\
        <input ' + (searchToggles.albums === true ? 'checked' : '') + ' id="search-in-albums" type="checkbox"><label for="search-in-albums">专辑</label><br>\
        <input ' + (searchToggles.titles === true ? 'checked' : '') + ' id="search-in-titles" type="checkbox"><label for="search-in-titles">歌名</label>\
        <input ' + (searchToggles.files === true ? 'checked' : '') + ' id="search-in-filepaths" type="checkbox"><label for="search-in-filepaths">文件路径</label>\
      </div>\
  ...
  $('.auto_dj_settings').on('click', function () {
    ...
    resetPanel('自动 DJ', 'scrollBoxHeight2');
    currentBrowsingList = [];
    $('#directory_bar').hide();

    var newHtml = '<br><p>自动DJ 随机生成一个播放列表.  单击底部的 \'DJ\' 按钮启动它</p><h3>使用的文件夹</h3><p>';
    for (var i = 0; i < MSTREAMAPI.currentServer.vpaths.length; i++) {
      var checkedString = '';
    ...
    newHtml += '</p><h3>最低评分</h3>  <select id="autodj-ratings">';
    for (var i = 0; i < 11; i++) {
      var selectedString = (Number(MSTREAMPLAYER.minRating) === i) ? 'selected' : '';
      var optionString = (i ===0) ? '禁用' : +(i/2).toFixed(1) ;
  ...
  $('.transcode-panel').on('click', function () {
    ...
    resetPanel('转码', 'scrollBoxHeight2');
    currentBrowsingList = [];
    $('#directory_bar').hide();

    var newHtml = "<p><b>转码是实验性功能</b></p>\
      <p>歌曲的位置和搜索不能正常运行。它可能无法在所有浏览器中都起作用。请向 <a target=\"_blank\"  href=\"https://github.com/IrosTheBeggar/mStream/issues/213\">反馈错误问题</a></p>";

    if (!MSTREAMAPI.transcodeOptions.serverEnabled) {
      newHtml += '<p>转码无法在此服务器运行</p>';
      $('#filelist').html(newHtml);
      return;
    }

    newHtml += '<p>默认比特率: '+MSTREAMAPI.transcodeOptions.bitrate+'</p>\
      <p>默认码率: '+MSTREAMAPI.transcodeOptions.codec+'</p>';
  ...
  $('.federation-panel').on('click', function () {
    ...
    resetPanel('连结', 'scrollBoxHeight2');
    currentBrowsingList = [];
    $('#directory_bar').hide();

    var newHtml = '\
      <p>连结允许用户可以轻松地在mStream服务器或备份工具之间同步文件夹。连结是一个单向过程。当用户邀请其他人时，其他人只能读取连结的文件夹。他们所做的任何更改都不会发送到您的mStream服务器。</p>\
      <p>连结功能版权为 <a target="_blank" href="https://syncthing.net/">Syncthing</a></p>';

    if (federationId) {
      newHtml += '\
      <p>连结 ID: <b class="autoselect">'+federationId+'</b></p>\
      <p><a href="#" class="trigger-generate-invite trigger-generate-invite-private">安全邀请</a> - 生成只能用于特定实例的邀请令牌。您将需要该计算机的连结身份验证ID。您的服务器无需公开可用即可运行。</p>\
      <p><a href="#" class="trigger-generate-invite trigger-generate-invite-public">公共邀请</a> - 生成任何人都可以用来访问您的连结文件夹的邀请令牌。 您的服务器必须可以公开使用。</p>\
      <p><a href="#" class="trigger-accept-invite">接收邀请</a> - 有邀请码?  这将验证它并完成连结过程</p>';
    }else {
      newHtml += '<p><b>连结处于禁用状态</b></p>';
    }
  ...
  function setupJukeboxPanel() {
    ...
    resetPanel('点播模式', 'scrollBoxHeight2');
    currentBrowsingList = [];
    $('#directory_bar').hide();
    ...
    if (JUKEBOX.stats.live !== false && JUKEBOX.connection !== false) {
      ...
      newHtml = '\
        ...
        <br><br>\
        <h3>点播模式可让您远程控制此页面<h3> <br><br>\
        <input value="连接" type="button" class="jukebox_connect">\
        </p>\
  ...
  function createJukeboxPanel() {
    ...
    if (JUKEBOX.stats.adminCode) {
      returnHtml += '<h1>编号: ' + JUKEBOX.stats.adminCode + '</h1>';
    }
    if (JUKEBOX.stats.guestCode) {
      returnHtml += '<h2>访客编号: ' + JUKEBOX.stats.guestCode + '</h2>';
    }

    var adrs = window.location.protocol + '//' + window.location.host + '/remote';
    returnHtml += '<br><h4>远程连接点播控制台: <a target="_blank" href="' + adrs + '"> ' + adrs + '</a><h4>';
